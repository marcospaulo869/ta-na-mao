...
- **2026-02-27** — **Materiais de investidor (privado)**: criados `/app/memory/ONE_PAGER_INVESTIDOR.md`, `/app/memory/PITCH_INVESTIDOR.md` (roteiro de 6 atos + objeções), `/app/memory/BLINDAGEM_IP.md` (5 camadas: INPI marca+software+patente método, contratos NDA/SCP, moats de dados/rede/marca/velocidade, PCT internacional Ano 3+). Rota privada `/apresentacao-privada` com gate de senha (passcode: `madeiraforte2026`) e 11 slides cinematográficos (Cover → Dor → Custo → Solução → Momento mágico → Mercado → Projeção 5 anos → Por que agora → Moat → Oferta SCP → Fechamento). Navegação por teclado + swipe mobile. Roadmap financeiro 5 anos: Ano 5 = R$ 14M receita, R$ 85–100M valuation.
