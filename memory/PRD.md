...
- **2026-02-27** — **Mic por Field (P1)**: componente `FieldMicButton.jsx` integrado em cada `Field` ao lado do toggle cm/mm. Novo endpoint backend `POST /api/voice/parse-number` (Whisper + GPT-5.4-mini com prompt `NUMBER_EXTRACTION_SYSTEM`) retorna `{transcription, value: float|null}`. Botão grande "IA · DITADO" preservado no topo do formulário — os dois coexistem. **Behavior press-and-hold estilo WhatsApp** (aperta pra gravar, solta pra enviar; auto-para em 8s).
- **2026-02-28** — **Diagnóstico conversa real Marcos↔Léo** salvo em `/app/memory/DIAGNOSTICO_CONVERSA_LEO.md`. Léo era profissional inaugurando consultório de neuropsicopedagogia infantil em Torres (não sala residencial). Pediu 2× o orçamento das 2 peças, Marcos entregou modelo comercial + credibilidade + indicação 10%. Cliente saiu "conversar com parceiro" (ghosting educado). Diagnóstico principal: **"pediu um número, Marcos entregou uma filosofia"**. Lista completa de 10+ dores mapeadas por dimensão (comunicação, orçamento, ferramenta, prazo, handoff, onboarding, referência visual).
- **2026-02-28** — **Estimador Rápido de Orçamento (P0)**: novo módulo `estimate.py`, endpoint `GET /api/estimate/catalog` + `POST /api/estimate`. 6 tipos de módulo (prateleira, mesa, balcão aberto, balcão portas de correr, armário completo, painel ripado) × 3 tiers MDF (básico R$180/m², médio R$280/m², premium R$420/m²) × largura+altura+profundidade+qty. Fórmula: material × complexity + fixed_hw + labor (1×). Devolve faixa min/avg/max + `whatsapp_text` já pronto para envio com saudação personalizada. Frontend `/orcamento-rapido` (protegido) com formulário + card de resultado + botões `btn-copy` e `btn-whatsapp` (abre wa.me/DDD).
- **2026-02-28** — **Referências do Cliente (P0)**: coleção `client_references` no Mongo. Endpoints em projects.py: `POST/GET/DELETE /api/projects/{id}/references`. Suporta 2 tipos: imagem (base64 dataURL, resized client-side para 1600px max) ou URL (Pinterest/GPT/etc). Componente `ClientReferences.jsx` mostra grid com lightbox, botões `btn-add-image` (file picker + resize + upload) e `btn-add-url` (paste URL + caption). Integrado em `ProjetoDetalhe.jsx`. Testado 18/19 backend + validação Playwright completa.
